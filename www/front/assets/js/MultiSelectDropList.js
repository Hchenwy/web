   ...
   $.fn.extend({
        MSDL: function (options){/*MultiSelectDropList*/
		 ...
		 //插件实现代码
			...
			var $ipt = $('<input type="text" readonly value="" class="select_rel form-control" required/>');
			$ipt.width(options.width-2);//设定文本框宽度
            ...
            var $btn = $('<button class="ok btn btn-success">'+options.selectokTxt+'</button>');
            $all.appendTo($top);
            $btn.appendTo($top);
